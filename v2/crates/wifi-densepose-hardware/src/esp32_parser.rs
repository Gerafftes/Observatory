...
/// Maximum UDP datagram size accepted by the ADR-018 parser, including the
/// optional TX-binding trailer.
pub const ESP32_CSI_MAX_DATAGRAM_SIZE: usize =
    HEADER_SIZE + MAX_ANTENNAS as usize * MAX_SUBCARRIERS * 2 + TX_BINDING_TRAILER_SIZE;

/// Magic at the start of the optional TX-binding trailer.
const TX_BINDING_TRAILER_MAGIC: u32 = 0xC511A060;
/// Supported TX-binding trailer version.
const TX_BINDING_TRAILER_VERSION: u8 = 1;
/// Exact TX-binding trailer size: header (8 bytes) + SHA-256 (32 bytes).
const TX_BINDING_TRAILER_SIZE: usize = 40;
/// Complete proof: filter enforced, source matched filter, identity valid.
const TX_BINDING_TRAILER_REQUIRED_FLAGS: u8 = 0x07;
...
/// Return the size of a structurally valid TX-binding trailer.
///
/// An incomplete or malformed trailer is deliberately left unconsumed so
/// legacy concatenated streams and arbitrary trailing bytes remain compatible.
fn tx_binding_trailer_size(data: &[u8]) -> usize {
    if data.len() < TX_BINDING_TRAILER_SIZE {
        return 0;
    }
    ...
    let version = data[4];
    let flags = data[5];
    let trailer_len = u16::from_le_bytes([data[6], data[7]]) as usize;
    let digest = &data[8..TX_BINDING_TRAILER_SIZE];
    let binding_semantics_are_valid = (flags == 0 && digest.iter().all(|byte| *byte == 0))
        || (flags == TX_BINDING_TRAILER_REQUIRED_FLAGS && digest.iter().any(|byte| *byte != 0));

    if magic == TX_BINDING_TRAILER_MAGIC
        && version == TX_BINDING_TRAILER_VERSION
        && trailer_len == TX_BINDING_TRAILER_SIZE
        && binding_semantics_are_valid
    {
        TX_BINDING_TRAILER_SIZE
    } else {
        0
    }
}
...
impl Esp32CsiParser {
    ...
    pub fn parse_frame(data: &[u8]) -> Result<(CsiFrame, usize), ParseError> {
        ...
        let consumed = total_frame_size + tx_binding_trailer_size(&data[total_frame_size..]);
        Ok((frame, consumed))
    }
...
mod tests {
    ...
    fn build_tx_binding_trailer(flags: u8) -> Vec<u8> {
        let mut trailer = Vec::with_capacity(TX_BINDING_TRAILER_SIZE);
        trailer.extend_from_slice(&TX_BINDING_TRAILER_MAGIC.to_le_bytes());
        trailer.push(TX_BINDING_TRAILER_VERSION);
        trailer.push(flags);
        trailer.extend_from_slice(&(TX_BINDING_TRAILER_SIZE as u16).to_le_bytes());
        if flags == 0 {
            trailer.extend_from_slice(&[0; 32]);
        } else {
            trailer.extend(1u8..=32);
        }
        trailer
    }
    ...
    #[test]
    fn test_parse_frame_consumes_valid_tx_binding_trailer() {
        for flags in [0x00, TX_BINDING_TRAILER_REQUIRED_FLAGS] {
            let mut data = build_test_frame(1, 1, &[(10, 20); 4]);
            let frame_size = data.len();
            data.extend_from_slice(&build_tx_binding_trailer(flags));

            let (frame, consumed) = Esp32CsiParser::parse_frame(&data).unwrap();

            assert_eq!(frame.metadata.node_id, 1);
            assert_eq!(consumed, frame_size + TX_BINDING_TRAILER_SIZE);
        }
    }

    #[test]
    fn test_parse_frame_does_not_consume_malformed_tx_binding_trailer() {
        let frame = build_test_frame(1, 1, &[(10, 20); 4]);
        let frame_size = frame.len();

        let mut malformed = Vec::new();

        let mut wrong_magic = build_tx_binding_trailer(0);
        wrong_magic[0] ^= 0x01;
        malformed.push(wrong_magic);

        let mut wrong_version = build_tx_binding_trailer(0);
        wrong_version[4] = TX_BINDING_TRAILER_VERSION + 1;
        malformed.push(wrong_version);

        let mut unknown_flags = build_tx_binding_trailer(0);
        unknown_flags[5] = 0x08;
        malformed.push(unknown_flags);

        for partial_flags in [0x01, 0x02, 0x04, 0x03, 0x05, 0x06] {
            malformed.push(build_tx_binding_trailer(partial_flags));
        }

        let mut unsealed_nonzero_digest = build_tx_binding_trailer(0);
        unsealed_nonzero_digest[8] = 1;
        malformed.push(unsealed_nonzero_digest);

        let mut sealed_zero_digest = build_tx_binding_trailer(TX_BINDING_TRAILER_REQUIRED_FLAGS);
        sealed_zero_digest[8..].fill(0);
        malformed.push(sealed_zero_digest);

        let mut wrong_length = build_tx_binding_trailer(0);
        wrong_length[6..8].copy_from_slice(&39u16.to_le_bytes());
        malformed.push(wrong_length);

        let mut truncated = build_tx_binding_trailer(0);
        truncated.pop();
        ...
        for trailer in malformed {
            ...
            let (_, consumed) = Esp32CsiParser::parse_frame(&data).unwrap();
    ...
    #[test]
    fn test_maximum_frame_with_tx_binding_trailer_fits_receive_buffer() {
        let mut data = build_test_frame(
            1,
            MAX_ANTENNAS,
            &[(0, 0); MAX_ANTENNAS as usize * MAX_SUBCARRIERS],
        );
        data.extend_from_slice(&build_tx_binding_trailer(TX_BINDING_TRAILER_REQUIRED_FLAGS));

        let (_, consumed) = Esp32CsiParser::parse_frame(&data).unwrap();

        assert_eq!(data.len(), ESP32_CSI_MAX_DATAGRAM_SIZE);
        assert_eq!(consumed, ESP32_CSI_MAX_DATAGRAM_SIZE);
    }
    ...
    #[test]
    fn test_parse_stream_consumes_tx_binding_trailers_between_frames() {
        let pairs = [(10, 20); 4];
        let mut trailer = build_tx_binding_trailer(TX_BINDING_TRAILER_REQUIRED_FLAGS);
        trailer[8..12].copy_from_slice(&ESP32_CSI_MAGIC.to_le_bytes());

        let mut combined = build_test_frame(1, 1, &pairs);
        combined.extend_from_slice(&trailer);
        combined.extend_from_slice(&build_test_frame(2, 1, &pairs));
        combined.extend_from_slice(&build_tx_binding_trailer(0));

        let (frames, consumed) = Esp32CsiParser::parse_stream(&combined);
    ...
    #[test]
    fn test_parse_stream_resynchronizes_after_a_truncated_unsealed_trailer() {
        let pairs = [(10, 20); 4];
        let mut combined = build_test_frame(1, 1, &pairs);
        let mut truncated = build_tx_binding_trailer(0);
        truncated.pop();
        combined.extend_from_slice(&truncated);
        combined.extend_from_slice(&build_test_frame(2, 1, &pairs));
