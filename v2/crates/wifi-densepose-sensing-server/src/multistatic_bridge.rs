...
pub fn node_frame_from_state(node_id: u8, ns: &NodeState) -> Option<MultiBandCsiFrame> {
    ...
    // Prefer ADR-110 mesh-aligned capture time when available. Falling back to
    // host arrival time keeps older firmware working, but arrival jitter should
    // not dominate synced ESP32 meshes.
    let timestamp_us = ns.latest_frame_mesh_time_us.unwrap_or_else(|| {
        last_time
