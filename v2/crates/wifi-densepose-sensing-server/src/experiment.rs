...
fn normalize_profile_document(document: &Value) -> Result<Value, String> {
    ...
    if !inside_room(tx_position, dimensions) {
        return Err("transmitter.position_m must be inside room_dimensions_m".to_string());
    }
    ...
    for (index, receiver) in receivers.iter().enumerate() {
        ...
        if !inside_room(position, dimensions) {
            return Err(format!("{expected_id}.position_m must be inside room_dimensions_m"));
        }
    }
    let points = object
        .get("points")
...
fn inside_room(position: [f64; 3], dimensions: [f64; 3]) -> bool {
    position
        .iter()
        .zip(dimensions.iter())
        .all(|(coordinate, dimension)| *coordinate >= 0.0 && *coordinate <= *dimension)
}
