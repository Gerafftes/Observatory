...
pub fn maybe_feed_calibration(field: &mut FieldModel, frame_history: &VecDeque<Vec<f64>>) {
    ...
    if let Some(latest) = frame_history.back() {
        let n_subcarriers = single_link_config().n_subcarriers;
        let mut normalized_frame: Vec<f64> = latest.iter().take(n_subcarriers).copied().collect();
        normalized_frame.resize(n_subcarriers, 0.0);
        // Single-link observation: [1][n_subcarriers]
        let observations = vec![normalized_frame];
...
mod tests {
    ...
    #[test]
    fn first_calibration_frame_starts_collection() {
        let mut field = FieldModel::new(single_link_config()).unwrap();
        let mut history = VecDeque::new();
        history.push_back(vec![1.0; 128]);
