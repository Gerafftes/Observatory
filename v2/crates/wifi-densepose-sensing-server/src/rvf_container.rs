...
impl RvfBuilder {
    ...
    /// Add a caller-supplied manifest. Torso models use this to carry their
    /// complete input, normalization, output, validation, and integrity contract.
    pub fn add_manifest_value(&mut self, manifest: &serde_json::Value) {
        let payload = serde_json::to_vec(manifest).unwrap_or_default();
        self.push_segment(SEG_MANIFEST, &payload);
    }
