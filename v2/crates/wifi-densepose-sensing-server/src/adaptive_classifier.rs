...
const RAW_CSI_RECORDING_SUFFIX: &str = ".raw-csi.v1.jsonl";

fn is_legacy_training_recording(file_name: &str) -> bool {
    file_name.starts_with("train_")
        && file_name.ends_with(".jsonl")
        && !file_name.ends_with(RAW_CSI_RECORDING_SUFFIX)
}
...
pub fn train_from_recordings(recordings_dir: &Path) -> Result<AdaptiveModel, String> {
    ...
    for entry in &entries {
        ...
        // Raw-v1 files contain exact I/Q pairs, not the legacy `features` and
        // `nodes[].amplitude` fields consumed by features_from_frame. Treating
        // them as legacy samples would silently produce all-zero features.
        if !is_legacy_training_recording(&fname) {
            continue;
        }
...
#[cfg(test)]
mod raw_recording_compatibility_tests {
    use super::*;

    #[test]
    fn raw_v1_suffix_is_not_a_legacy_training_recording() {
        assert!(!is_legacy_training_recording(
            "train_empty.raw-csi.v1.jsonl"
        ));
        assert!(is_legacy_training_recording("train_empty.csi.jsonl"));
        assert!(!is_legacy_training_recording("empty.csi.jsonl"));
    }
}
