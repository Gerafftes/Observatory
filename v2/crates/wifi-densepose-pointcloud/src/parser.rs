...
mod tests {
    ...
    #[test]
    fn parse_v1_ignores_tx_binding_trailer_after_iq_payload() {
        let mut frame_bytes = build_test_frame(MAGIC_V1, 0x42, 56, 7);
        frame_bytes.extend_from_slice(&0xC511A060u32.to_le_bytes());
        frame_bytes.push(1);
        frame_bytes.push(0x07);
        frame_bytes.extend_from_slice(&40u16.to_le_bytes());
        frame_bytes.extend_from_slice(&[0xAB; 32]);

        let frame = parse_adr018(&frame_bytes).expect("trailed v1 frame should parse");
