...
> unter [`/results`](../../experiment-reports/README.md) gepflegt.
...
| mmWave-Referenz | Laufzeitprüfung nicht bestanden | Der geprüfte Lauf akzeptierte 179 Pakete, enthielt aber eine Sequenzlücke von 66 Paketen und eine Unterbrechung von 25,4 s; der [Runtime-Audit](results/2026-08-30_mmwave-runtime-audit.md) bewertete das Fenster als `FAIL`. |
...
## Sicherheitsstatus und frühere Lücken

**Stand: 2. September 2026**

Eine Sicherheitsprüfung des verwendeten RuView-/HomeCore-Runtimepfads enthielt
13 Listeneinträge. Der WebSocket-Befund stand in zwei Zeilen und der
REST-Befund ebenfalls in zwei Zeilen; damit blieben elf eigenständige Befunde.
Im Ausgangszustand bestanden diese Lücken:

- **ESP32-CSI-Knoten:** Der Konfigurationsendpunkt und persistente
  Konfigurationsschreibvorgänge waren ohne Authentifizierung erreichbar.
- **ESP32-WASM/OTA:** WASM-Lebenszyklus-Endpunkte waren nicht ausreichend
  autorisiert; beim OTA-bezogenen Pfad fehlte die Autorisierungsprüfung.
- **RVF-WASM-Signaturen:** Die Prüfung war ohne den privaten Schlüssel
  fälschbar und damit keine belastbare Signaturprüfung.
- **RuField:** Live-Ereignisse fielen auf einen öffentlich ableitbaren
  Standardschlüssel zurück.
- **HomeCore:** Der Netzwerkserver beziehungsweise die API akzeptierte
  standardmäßig triviale oder beliebige nichtleere Bearer-Tokens.
- **Sensing-Bridge:** Live-Telemetrie war ohne erforderlichen Bearer-Schutz
  erreichbar.
- **Browser-Origin-Schutz:** Die hostbasierte Prüfung behandelte `localhost`
  auf beliebigen Ports als gleich. Eine Seite von beispielsweise
  `http://localhost:3000` konnte deshalb den Dienst auf einem anderen lokalen
  Port über WebSocket und mutierende REST-Aufrufe ansprechen.

Der aktuelle Quellstand enthält Gegenmaßnahmen für diese Befunde: ESP32-
Schreib-, WASM- und OTA-Flächen schließen ohne PSK fehl; RVF verwendet echte
Ed25519-Signaturen; RuField startet nicht ohne einen deploymentspezifischen
Schlüssel; HomeCore akzeptiert nur explizit provisionierte Tokens; die
Sensing-Bridge und nichtlokale Sensing-Binds verlangen einen Bearer-Schutz.
Browser-WebSockets und mutierende REST-Aufrufe verwenden eine exakte
Origin-Allowlist, getrennt von der Host-/DNS-Rebinding-Prüfung. Standardmäßig
sind nur die lokalen UI-Origins des konfigurierten HTTP-Ports erlaubt; eine
separat laufende UI muss ihre vollständige Origin explizit konfigurieren.

Die Quellcode- und Host-Verifikation bestand: Origin-Tests `24/24`, vollständige
Sensing-Server-Suite `471 bestanden`, HomeCore `18 + 18 + 6 + 5` Tests sowie
Python-/Firmware-Sicherheitsgrenztests `7 bestanden`. Das ersetzt keinen
nativen Firmware-Build. Die ESP-IDF-v5.4-Matrix für ESP32-S3 8 MB, ESP32-S3
4 MB und ESP32-C6 4 MB blieb in der lokalen Umgebung `inconclusive`; es wurde
nichts geflasht und keine OTA-Aktion ausgeführt. Deshalb wird hier keine
vollständige Firmware- oder Produktionsfreigabe behauptet.
