...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    memcpy(info.mac, data, sizeof(info.mac));

    /* Exercise both unfiltered and runtime-filter-attested trailers. */
    memset(&g_nvs_config, 0, sizeof(g_nvs_config));
    g_nvs_config.filter_mac_set = (test_case & 0x80) != 0;
    memcpy(g_nvs_config.filter_mac, data, sizeof(g_nvs_config.filter_mac));
    if ((legacy_inputs & 0x80) != 0) {
        info.mac[0] ^= 0xff;  /* Filter configured, source deliberately differs. */
    }
    csi_collector_set_node_id(1);
    ...
    if (test_case & 0x20) {
        ...
        /* len=0 means header + fixed source-binding trailer. */
        if (result != 0
            && result != CSI_HEADER_SIZE + CSI_SOURCE_BINDING_SIZE) {
            __builtin_trap();
        }
    }

    /* --- Test case 7: Output buffer exactly header + I/Q + trailer size --- */
    if (test_case & 0x40) {
        wifi_csi_info_t hdr_info = info;
        hdr_info.buf = iq_buf;
        hdr_info.len = 4;  /* Small I/Q. */
        uint8_t tight_buf[
            CSI_HEADER_SIZE + 4 + CSI_SOURCE_BINDING_SIZE
        ];
