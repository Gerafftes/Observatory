...
Für `--kind empty` sind zusätzlich `--profile-id`, die unveränderliche
`--profile-revision-id` und `--confirm-empty-room` erforderlich. Ein Start
ohne diesen Run-/Setup-Kontext wird abgewiesen. Das Werkzeug:
...
- meldet den Leerraumlauf erst nach mindestens 60 Sekunden, vier vollständigen
  RX-Referenzen und einem persistenten Bundle mit passender Profilrevision,
  `calibration_id` und `calibration_context_sha256` als bestanden
...
Die Radar-Runtime wird separat mit Status-Snapshots aus demselben Serverprozess
geprüft. Das ist nötig, weil serverseitig verworfene Pakete wie `room_bounds`
nicht in der akzeptierten mmWave-Session-JSONL stehen:

```bash
curl -fsS http://127.0.0.1:8080/api/v1/mmwave/status \
  -o /tmp/mmwave-status-before.json
# 25-s-Preflight oder mmWave-Session durchführen.
curl -fsS http://127.0.0.1:8080/api/v1/mmwave/status \
  -o /tmp/mmwave-status-after.json
python3 scripts/audit_mmwave_runtime.py \
  --recording data/mmwave/<SESSION>.mmwave.jsonl \
  --setup /absoluter/pfad/sealed-setup.json \
  --status-before /tmp/mmwave-status-before.json \
  --status-after /tmp/mmwave-status-after.json
```

Der Auditor verlangt eine zum Seal passende Node-/Transform-Identität,
rechnet alle Raumkoordinaten aus den Radar-Rohwerten nach und weist
`room_bounds`, Radar-Reboots, Sequenzlücken und Clock-Wechsel getrennt aus.

Beim mmWave-geführten Blindtest wird neben der global fusionierten
WiFi-only-Entscheidung für jeden Besuch auch eine diagnostische
Nearest-Prototype-Ablation für RX1 bis RX4 **vor** dem Radar-Truth-Reveal im
Prediction-Artefakt eingefroren. Der Evaluationsbericht enthält damit globale
Metriken sowie Trefferquote, Median- und Maximalfehler je RX. Die
RX-Ablationen sind Diagnosewerte; nur die fusionierte Vier-RX-Entscheidung
steuert den PASS-/Deployment-Gate.
