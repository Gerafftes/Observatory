...
/// UDP receive buffer for the largest supported raw CSI frame plus its
/// optional 40-byte TX-binding trailer.
...
mod tests {
    ...
    #[test]
    fn test_parse_csi_packet_accepts_tx_binding_trailer() {
        let mut buf = build_frame(256, 1);
        buf.extend_from_slice(&0xC511A060u32.to_le_bytes());
        buf.push(1);
        buf.push(0x07);
        buf.extend_from_slice(&40u16.to_le_bytes());
        buf.extend_from_slice(&[0xAB; 32]);

        let frame = parse_csi_packet(&buf, "he20").expect("trailed frame must parse");

        assert_eq!(frame.num_spatial_streams(), 1);
        assert_eq!(frame.num_subcarriers(), 256);
    }

    #[test]
    fn test_receive_buffer_fits_maximum_supported_trailed_frame() {
        const MAX_TRAILED_FRAME_SIZE: usize = 20 + 4 * 256 * 2 + 40;
        assert!(RECV_BUF >= MAX_TRAILED_FRAME_SIZE);
    }
    ...
    /// a RECV_BUF-sized datagram parses to `None`, never OOMs.
    #[test]
    fn test_parse_csi_packet_oversized_claim_is_rejected_not_allocated() {
        ...
        // The bounded datagram cannot carry it → length check fails → None.
        assert!(parse_csi_packet(&buf, "ht20").is_none());
    }
