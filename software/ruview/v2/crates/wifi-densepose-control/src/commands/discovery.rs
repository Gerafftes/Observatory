...
pub async fn configure_esp32_wifi(
    ...
    validate_serial_port_path(&port)?;
...
/// Restrict browser-provided serial paths to device names that can actually be
/// returned by the local enumerator. The helper must never open an arbitrary
/// path supplied by a webpage.
pub fn validate_serial_port_path(port: &str) -> Result<(), String> {
    let valid = if cfg!(target_os = "windows") {
        ...
        path.parent() == Some(std::path::Path::new("/dev"))
            && path
                .file_name()
                .and_then(|name| name.to_str())
                .is_some_and(|name| name.starts_with("cu.") || name.starts_with("tty."))
    } else {
        let path = std::path::Path::new(port);
        let is_direct_device = path.parent() == Some(std::path::Path::new("/dev"))
            && path
                .file_name()
                .and_then(|name| name.to_str())
                .is_some_and(|name| name.starts_with("ttyUSB") || name.starts_with("ttyACM"));
        let is_stable_symlink = path
            .parent()
            .and_then(|parent| parent.to_str())
            .is_some_and(|parent| parent == "/dev/serial/by-id")
            && path.file_name().is_some_and(|name| !name.is_empty());
        is_direct_device || is_stable_symlink
    };

    if valid {
        Ok(())
    } else {
        Err(format!("Serieller Port ist nicht erlaubt: {port}"))
    }
}
...
mod tests {
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn test_validate_serial_port_path_accepts_device_names_only() {
        assert!(validate_serial_port_path("/dev/cu.usbserial-0001").is_ok());
        assert!(validate_serial_port_path("/dev/tty.usbmodem-0001").is_ok());
        assert!(validate_serial_port_path("/dev/cu../private/secret").is_err());
        assert!(validate_serial_port_path("/dev/cu.usbserial/../secret").is_err());
        assert!(validate_serial_port_path("/tmp/cu.usbserial").is_err());
    }
}
