...
use crate::esp32_parser::{Esp32CsiParser, ESP32_CSI_MAX_DATAGRAM_SIZE};
...
impl Esp32Aggregator {
    ...
    pub fn run(&mut self) -> io::Result<()> {
        let mut buf = [0u8; ESP32_CSI_MAX_DATAGRAM_SIZE];
        loop {
...
mod tests {
    ...
    fn build_test_packet(node_id: u8, sequence: u32, n_subcarriers: usize) -> Vec<u8> {
        build_test_packet_with_antennas(node_id, sequence, 1, n_subcarriers)
    }

    fn build_test_packet_with_antennas(
        node_id: u8,
        sequence: u32,
        n_antennas: u8,
        n_subcarriers: usize,
        ...
        // I/Q data
        for i in 0..n_antennas as usize * n_subcarriers {
    ...
    #[test]
    fn test_aggregator_loopback_udp_receives_maximum_trailed_frame() {
        let recv_socket = UdpSocket::bind("127.0.0.1:0").unwrap();
        let recv_addr = recv_socket.local_addr().unwrap();
        recv_socket
        ...
        let mut packet = build_test_packet_with_antennas(4, 77, 4, 256);
        packet.extend_from_slice(&0xC511A060u32.to_le_bytes());
        packet.push(1);
        packet.push(0x07);
        packet.extend_from_slice(&40u16.to_le_bytes());
        packet.extend_from_slice(&[0xAB; 32]);
        assert_eq!(packet.len(), ESP32_CSI_MAX_DATAGRAM_SIZE);

        send_socket.send_to(&packet, recv_addr).unwrap();

        let mut buf = [0u8; ESP32_CSI_MAX_DATAGRAM_SIZE];
        let (n, _) = agg.socket.recv_from(&mut buf).unwrap();
        assert_eq!(n, packet.len());
        agg.handle_packet(&buf[..n]);

        let frame = rx.try_recv().unwrap();
        assert_eq!(frame.metadata.node_id, 4);
        assert_eq!(frame.metadata.sequence, 77);
        assert_eq!(frame.metadata.n_antennas, 4);
