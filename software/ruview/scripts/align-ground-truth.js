...
const { parseIqHex: parsePreprocessingIqHex, preprocessIq } = require('./csi-preprocessing');
const {
  RX_ORDER,
  TORSO_KEYPOINTS,
  TORSO_SCHEMA,
...
Options:
  ...
  --task <pose|torso>        Output legacy 17-keypoint or torso-v1 samples (default: pose)
  --session-id <id>          Required recording-session group for --task torso
...
if (!['pose', 'torso'].includes(TASK)) {
  throw new Error(`Unsupported task '${TASK}'; expected pose or torso`);
}
if (TASK === 'torso' && !args['session-id']) {
  throw new Error('--session-id is required for --task torso');
}
...
function averageTorso(cameraFrames) {
  const sums = Object.fromEntries(TORSO_KEYPOINTS.map(name => [name, { x: 0, y: 0, weight: 0 }]));
  for (const frame of cameraFrames) {
    const torso = torsoFromCoco(frame.keypoints, frame.confidence);
    for (const name of TORSO_KEYPOINTS) {
      const point = torso[name];
      const weight = Math.max(0, point.confidence || frame.confidence || 0);
      sums[name].x += point.x * weight;
      sums[name].y += point.y * weight;
      sums[name].weight += weight;
    }
  }
  return Object.fromEntries(TORSO_KEYPOINTS.map(name => {
    const sum = sums[name];
    if (sum.weight === 0) throw new Error(`${name} has zero confidence in aligned window`);
    return [name, {
      x: sum.x / sum.weight,
      y: sum.y / sum.weight,
      confidence: sum.weight / cameraFrames.length,
    }];
  }));
}

function prepareTorsoFrames(frames) {
  const byRx = Object.fromEntries(RX_ORDER.map(id => [id, []]));
  for (const frame of frames) {
    const rxId = canonicalRxId(frame.nodeId);
    if (!rxId || frame.subcarriers !== 64) continue;
    if (frame.iqHex) {
      const processed = preprocessIq(parsePreprocessingIqHex(frame.iqHex), 64);
      byRx[rxId].push({ ...frame, amplitude: processed.amplitude, phase: processed.phase });
    } else if (Array.isArray(frame.amplitude) && frame.amplitude.length === 64) {
      byRx[rxId].push({ ...frame, amplitude: [...frame.amplitude], phase: null });
    }
  }
  return byRx;
}

function alignTorso(cameraFrames, rawCsi) {
  const framesByRx = prepareTorsoFrames(rawCsi);
  for (const rxId of RX_ORDER) {
    if (framesByRx[rxId].length < WINDOW_FRAMES) {
      throw new Error(`${rxId} has ${framesByRx[rxId].length} valid 64-subcarrier frames; torso-v1 requires at least ${WINDOW_FRAMES}`);
    }
  }

  const windowCount = Math.min(...RX_ORDER.map(rxId => Math.floor(framesByRx[rxId].length / WINDOW_FRAMES)));
  const paired = [];
  for (let windowIndex = 0; windowIndex < windowCount; windowIndex++) {
    const windowByRx = {};
    for (const rxId of RX_ORDER) {
      const start = windowIndex * WINDOW_FRAMES;
      windowByRx[rxId] = framesByRx[rxId].slice(start, start + WINDOW_FRAMES);
    }
    for (let time = 0; time < WINDOW_FRAMES; time++) {
      const frames = RX_ORDER.map(rxId => windowByRx[rxId][time]);
      const sequences = frames.map(frame => frame.seq);
      if (sequences.every(sequence => sequence != null)) {
        if (new Set(sequences).size !== 1) {
          throw new Error(`missing or misaligned RX frame at torso time point ${time}`);
        }
      } else {
        const timestamps = frames.map(frame => frame.tsMs);
        if (Math.max(...timestamps) - Math.min(...timestamps) > 50) {
          throw new Error(`RX timestamps are not synchronized at torso time point ${time}`);
        }
      }
    }
    const allFrames = RX_ORDER.flatMap(rxId => windowByRx[rxId]);
    const tStartMs = Math.min(...allFrames.map(frame => frame.tsMs));
    const tEndMs = Math.max(...allFrames.map(frame => frame.tsMs));
    const midpoint = (tStartMs + tEndMs) / 2;
    const matched = findCameraFramesInRange(
      cameraFrames,
      Math.min(tStartMs, midpoint - WINDOW_MS / 2),
      Math.max(tEndMs, midpoint + WINDOW_MS / 2),
    );
    if (matched.length < MIN_CAMERA_FRAMES) continue;
    const confidence = matched.reduce((sum, frame) => sum + frame.confidence, 0) / matched.length;
    if (confidence < MIN_CONFIDENCE) continue;

    const phaseAvailable = RX_ORDER.every(rxId => windowByRx[rxId].every(frame => Array.isArray(frame.phase)));
    paired.push({
      schema_version: TORSO_SCHEMA,
      validation_status: 'UNVALIDATED',
      session_id: args['session-id'],
      rx_order: [...RX_ORDER],
      csi_shape: [WINDOW_FRAMES, RX_ORDER.length, 64],
      csi_amplitude: flattenTimeMajorChannel(windowByRx, 'amplitude', WINDOW_FRAMES, 64),
      csi_phase: phaseAvailable
        ? flattenTimeMajorChannel(windowByRx, 'phase', WINDOW_FRAMES, 64)
        : null,
      phase_status: phaseAvailable ? 'sanitized' : 'unavailable',
      torso: averageTorso(matched),
      n_camera_frames: matched.length,
      ts_start: new Date(tStartMs).toISOString(),
      ts_end: new Date(tEndMs).toISOString(),
    });
  }
  return { paired, windowCount };
}
...
function align() {
  ...
  if (TASK === 'torso') {
    const { paired, windowCount } = alignTorso(cameraFrames, rawCsi);
    const outputLines = paired.map(sample => JSON.stringify(sample));
    fs.writeFileSync(outputPath, outputLines.join('\n') + (outputLines.length ? '\n' : ''));
    console.log(`  Torso windows:    ${windowCount}`);
    console.log(`  Paired samples:   ${paired.length}`);
    console.log(`  Status:           UNVALIDATED (software fixture path only)`);
