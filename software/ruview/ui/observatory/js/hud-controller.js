...
export class HudController {
  ...
  _ensureEvidenceUI() {
    ...
    if (hud && !document.getElementById('measurement-status')) {
      const status = document.createElement('div');
      status.id = 'measurement-status';
      status.className =
        'measurement-status measurement-status--simulated';

      const rows = [
        {
          className: 'measurement-status__eyebrow',
          text: 'Visualization evidence',
        },
        {
          id: 'measurement-mode-note',
          text: 'Local demonstration data; not an ESP32 measurement.',
        },
        {
          id: 'measurement-geometry-status',
          text: 'DEMO GEOMETRY',
        },
        {
          id: 'measurement-position-status',
          text: 'Procedural animated pose · not measured',
        },
        {
          id: 'measurement-field-note',
          text: 'Synthetic field · not a hardware position',
        },
      ];
      for (const row of rows) {
        const element = document.createElement('div');
        if (row.id) element.id = row.id;
        if (row.className) element.className = row.className;
    ...
    const signalPanel = document.getElementById('panel-signal');
    if (signalPanel && !signalPanel.querySelector('.field-disclaimer')) {
      const disclaimer = document.createElement('div');
      disclaimer.className = 'field-disclaimer';
      disclaimer.textContent =
        'Signal field is diagnostic CSI data, not a measured person position.';
      signalPanel.appendChild(disclaimer);
    }

    const capability = document.querySelector(
      '#capabilities-bar .cap-item span:last-child'
    );
    if (capability) {
      capability.id = 'primary-capability-label';
      capability.textContent = 'Simulated Pose';
    }
  }
  ...
  initSettings() {
    ...
    this._bindRange('opt-field', 'field', v => {
      obs._fieldMat.opacity = v;
      if (obs._hardwareFieldPoints) obs._hardwareFieldPoints.material.opacity = v;
    });
    ...
    this._bindRange('opt-reflect', 'reflect', v => {
      ...
      if (obs._hardwareFloorMat) {
        obs._hardwareFloorMat.roughness = 1.0 - v * 0.7;
        obs._hardwareFloorMat.metalness = v * 0.5;
      }
    });
    ...
    document.getElementById('opt-grid').addEventListener('change', (e) => {
      ...
      obs._grid.visible = e.target.checked && obs._sceneMode === 'simulated';
      if (obs._hardwareGrid) obs._hardwareGrid.visible = e.target.checked;
      this.saveSettings();
    });
    ...
    document.getElementById('opt-room').addEventListener('change', (e) => {
      ...
      obs._roomWire.visible = e.target.checked && obs._sceneMode === 'simulated';
      if (obs._hardwareRoomWire) obs._hardwareRoomWire.visible = e.target.checked;
      this.saveSettings();
    });
    ...
    dsSel.addEventListener('change', (e) => {
      ...
      this.updateSourceBadge(
        obs._currentViewState?.sourceState || {
          state: e.target.value === 'ws' ? 'connecting' : 'simulated',
          label: e.target.value === 'ws' ? 'CONNECTING' : 'SIMULATED',
        }
      );
      this.saveSettings();
    });
    ...
    obs._grid.visible = s.grid && obs._sceneMode === 'simulated';
    obs._roomWire.visible = s.room && obs._sceneMode === 'simulated';
  }
  ...
  applyPreset(preset) {
    ...
    if (gridEl) {
      gridEl.checked = obs.settings.grid;
      obs._grid.visible = obs.settings.grid && obs._sceneMode === 'simulated';
      if (obs._hardwareGrid) obs._hardwareGrid.visible = obs.settings.grid;
    }
    const roomEl = document.getElementById('opt-room');
    if (roomEl) {
      roomEl.checked = obs.settings.room;
      obs._roomWire.visible = obs.settings.room && obs._sceneMode === 'simulated';
      if (obs._hardwareRoomWire) obs._hardwareRoomWire.visible = obs.settings.room;
    }
    ...
    if (obs._hardwareFieldPoints) {
      obs._hardwareFieldPoints.material.opacity = obs.settings.field;
    }
    ...
    if (obs._hardwareFloorMat) {
      obs._hardwareFloorMat.roughness = 1.0 - obs.settings.reflect * 0.7;
      obs._hardwareFloorMat.metalness = obs.settings.reflect * 0.5;
    }
  ...
  updateSourceBadge(sourceState) {
    ...
    dot.className = `dot ${classByState[state] || 'dot--stale'}`;
    label.textContent = sourceState?.label || 'STALE';
  }
  ...
  updateHUD(data, demoData, viewState) {
    const sourceState = viewState?.sourceState || {
      state: 'stale',
      label: 'STALE',
      reason: 'No source state is available.',
    };
    const simulated = sourceState.state === 'simulated';
    const live = sourceState.state === 'live';
    if (sourceState.state !== this._currentSourceState) {
      this._currentSourceState = sourceState.state;
      this._rssiHistory = [];
      if (this._sparklineCtx) {
        const ctx = this._sparklineCtx;
        ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height);
      }
    }
    this.updateSourceBadge(sourceState);
    this._updateMeasurementStatus(viewState);
    this._setSimulationControlsEnabled(simulated);

    if (!data) {
      this._clearMeasurements(sourceState);
      return;
    }
    ...
    this._updateClassificationPanel({data, cls, sourceState, viewState});

    // Sync scenario dropdown
    const quickSel = document.getElementById('scenario-quick-select');
    const cur = data.scenario ||
      (demoData._autoMode ? 'auto' : demoData.currentScenario);
    if (simulated && quickSel && quickSel.value !== cur) quickSel.value = cur;
    const autoIcon = document.getElementById('autoplay-icon');
    if (autoIcon) {
      autoIcon.className = simulated && demoData._autoMode ? '' : 'hidden';
    }
    ...
    const personCount = live
      ? (viewState?.contract?.position?.valid ? 1 : 0)
    ...
    const scenarioKey = simulated
      ? (data.scenario || demoData.currentScenario || 'auto')
      : null;
  ...
  updateSparkline(data) {
    ...
    grad.addColorStop(0, 'rgba(17,17,17,0.12)');
  ...
  _updateMeasurementStatus(viewState) {
    const sourceState = viewState?.sourceState || {
      state: 'stale',
      reason: 'No source state is available.',
    };
    const status = document.getElementById('measurement-status');
    const modeNote = document.getElementById('measurement-mode-note');
    const geometryStatus = document.getElementById('measurement-geometry-status');
    const positionStatus = document.getElementById('measurement-position-status');
    const fieldNote = document.getElementById('measurement-field-note');
    const capability = document.getElementById('primary-capability-label');

    if (status) {
      status.className = `measurement-status measurement-status--${sourceState.state}`;
    }
    if (modeNote) modeNote.textContent = sourceState.reason || '';

    if (sourceState.state === 'simulated') {
      if (geometryStatus) geometryStatus.textContent = 'DEMO GEOMETRY';
      if (positionStatus) {
        positionStatus.textContent = 'Procedural animated pose · not measured';
      }
      if (fieldNote) {
        fieldNote.textContent = 'Synthetic field · not a hardware position';
      }
      if (capability) capability.textContent = 'Simulated Pose';
      return;
    }

    if (sourceState.state !== 'live') {
      if (geometryStatus) geometryStatus.textContent = 'HARDWARE HIDDEN';
      if (positionStatus) positionStatus.textContent = 'No current position';
      if (fieldNote) fieldNote.textContent = 'Hardware marker and field cleared';
      if (capability) capability.textContent = 'Position unavailable';
      return;
    }

    const geometry = viewState?.contract?.geometry;
    const position = viewState?.contract?.position;
    const field = viewState?.contract?.signalField;
    if (geometryStatus) {
      geometryStatus.textContent = geometry?.valid
        ? `MEASURED GEOMETRY · ${geometry.receivers.length} RX · TX blue / RX amber`
        : `GEOMETRY REJECTED · ${geometry?.reason || 'missing'}`;
    }
    if (positionStatus) {
      positionStatus.textContent = position?.valid
        ? `${position.pointId} · x ${position.coordinates[0].toFixed(2)} · y ${position.coordinates[1].toFixed(2)} · z ${position.coordinates[2].toFixed(2)} m`
        : `NO EXACT POSITION · ${position?.reason || 'missing'}`;
    }
    if (fieldNote) {
      fieldNote.textContent = viewState?.contract?.showHardwareField
        ? 'Diagnostic CSI field · not a person position'
        : `Field hidden · ${field?.reason || position?.reason || 'position gate failed'}`;
    }
    if (capability) capability.textContent = 'Neutral Position Marker';
  }

  _setSimulationControlsEnabled(enabled) {
    const scenarioArea = document.getElementById('scenario-area');
    if (scenarioArea) scenarioArea.style.display = enabled ? 'flex' : 'none';
    const scenarioDescription = document.getElementById('scenario-description');
    if (scenarioDescription) {
      scenarioDescription.style.display = enabled ? 'block' : 'none';
    }

    for (const id of ['scenario-quick-select', 'opt-scenario', 'opt-cycle']) {
      const element = document.getElementById(id);
      if (element) element.disabled = !enabled;
    }
  }

  _clearMeasurements(sourceState) {
    this._lerpHr = 0;
    this._lerpBr = 0;
    this._lerpConf = 0;
    ...
    if (presence) {
      presence.className = 'presence-state presence--pending';
    }
    if (presenceLabel) {
      presenceLabel.textContent =
        sourceState.state === 'connecting' ? 'WAITING FOR FRAME' : 'NO FRESH DATA';
    }

    const fallAlert = document.getElementById('fall-alert');
    if (fallAlert) fallAlert.style.display = 'none';
    this._updateClassificationPanel({data: {}, cls: {}, sourceState, viewState: null});
  }

  _updateClassificationPanel({data, cls, sourceState, viewState}) {
    const position = viewState?.contract?.position;
    const hasValidatedPosition = sourceState.state === 'live' && position?.valid;
    const confidence = Number(cls?.confidence);
    const timestamp = Number(data?.timestamp);
    const frameValue = data?.frame_id ?? data?.frame ?? data?.sequence;

    const designation = sourceState.label || sourceState.state || 'UNKNOWN';
    const projection = String(
      cls?.motion_level || (cls?.presence ? 'present' : 'absent'),
    ).replace(/_/g, ' ').toUpperCase();
    const subject = sourceState.state === 'simulated'
      ? 'SIMULATION'
      : hasValidatedPosition
        ? position.pointId || 'LIVE SUBJECT'
        : 'UNASSIGNED';
    const coordinate = hasValidatedPosition
      ? `X ${position.coordinates[0].toFixed(2)}  Y ${position.coordinates[1].toFixed(2)}  Z ${position.coordinates[2].toFixed(2)} M`
      : sourceState.state === 'simulated'
        ? 'NOT MEASURED'
        : 'UNAVAILABLE';
    ...
    if (Number.isFinite(timestamp) && timestamp > 0) {
      ...
      if (!Number.isNaN(date.getTime())) clock = `${date.toISOString().slice(11, 19)}Z`;
    }

    this._setText('classification-designation', designation);
    this._setText('classification-projection', projection);
    this._setText(
      'classification-confidence',
      Number.isFinite(confidence) ? `${Math.round(confidence * 100)}%` : '--',
    );
    this._setText('classification-subject', subject);
    this._setText('classification-coordinate', coordinate);
    this._setText(
      'classification-frame',
      `${frameValue === undefined || frameValue === null ? '--' : frameValue} / ${clock}`,
    );

    document.getElementById('classification-projection')?.classList.toggle(
      'critical',
      cls?.fall_detected === true,
    );
  }
