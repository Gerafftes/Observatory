    ...
    try {
      ...
      const payload = {
        dataset_ids: this.config.selectedRecordings.filter(id => {
          ...
          return recording?.trainable !== false;
        }),
        config: {
  ...
  _renderRecordings() {
    ...
    if (this.state.recordings.length === 0 && !this.state.loading) {
      ...
      this.state.recordings.forEach(rec => {
        ...
        if (rec.format === 'raw-csi-v1-jsonl') parts.push('Raw CSI / offline');
        if (rec.incomplete) parts.push(`Incomplete (${rec.dropped_frames || 0} dropped)`);
        ...
        del.disabled = this.state.loading || rec.status === 'recording' || rec.status === 'finalizing';
        row.appendChild(del);
        s.appendChild(row);
      });
    }
  ...
  _renderConfig() {
    ...
    if (this.state.recordings.length > 0) {
      ...
      this.state.recordings.forEach(rec => {
        ...
        cb.type = 'checkbox';
        cb.disabled = rec.trainable === false;
        ...
        lb.appendChild(cb);
        const datasetName = rec.trainable === false
          ? `${rec.name || rec.id} (Raw CSI – offline analysis)`
          : (rec.name || rec.id);
        lb.appendChild(this._el('span', null, datasetName));
        dc.appendChild(lb);
      });
      form.appendChild(dc);
    }
