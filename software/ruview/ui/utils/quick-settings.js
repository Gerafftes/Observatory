...
import { getHeaderActions } from './header-actions.js';
...
export class QuickSettings {
  ...
  createButton() {
    ...
    this.button.setAttribute('data-i18n-aria', 'misc.settings');
    this.button.setAttribute('title', 'Quick settings');
    this.button.setAttribute('data-i18n-title', 'misc.settings');
    this.button.innerHTML = `<span class="settings-gear-label" data-i18n="misc.settings">Settings</span><svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>`;

    this.button.addEventListener('click', () => this.toggle());

    const headerActions = getHeaderActions();
    if (headerActions) headerActions.appendChild(this.button);
  }
  ...
  createPanel() {
    ...
    this.panel.setAttribute('data-i18n-aria', 'settings.title');

    this.panel.innerHTML = `
      <div class="qs-header">
        <h3 data-i18n="settings.title">Settings</h3>
        <button class="qs-close" aria-label="Close" data-i18n-aria="misc.close">&times;</button>
      </div>
      <div class="qs-body">
        <div class="qs-section">
          <div class="qs-section-title" data-i18n="settings.display">Display</div>
          <label class="qs-toggle">
            <span data-i18n="settings.reducedMotion">Reduced motion</span>
          ...
          <label class="qs-toggle">
            <span data-i18n="settings.compactMode">Compact mode</span>
            <input type="checkbox" id="qs-compact" ${this.getSetting('compact') ? 'checked' : ''}>
            <span class="qs-switch"></span>
          </label>
        </div>
        <div class="qs-section">
          <div class="qs-section-title" data-i18n="settings.monitoring">Monitoring</div>
          <label class="qs-toggle">
            <span data-i18n="settings.healthPolling">Health polling</span>
            <input type="checkbox" id="qs-health-polling" checked>
            <span class="qs-switch"></span>
          </label>
          <label class="qs-toggle">
            <span data-i18n="settings.autoReconnect">Auto-reconnect</span>
            <input type="checkbox" id="qs-auto-reconnect" checked>
            <span class="qs-switch"></span>
          </label>
        </div>
        <div class="qs-section">
          <div class="qs-section-title" data-i18n="settings.data">Data</div>
          <div class="qs-row">
            <span data-i18n="settings.clearLocalData">Clear local data</span>
            <button class="qs-btn-danger" id="qs-clear-data" data-i18n="settings.clear">Clear</button>
          </div>
          <div class="qs-row">
            <span data-i18n="settings.resetOnboarding">Reset onboarding</span>
            <button class="qs-btn" id="qs-reset-tour" data-i18n="settings.reset">Reset</button>
          </div>
        </div>
      </div>
    `;
