...
import { RoomGeometryEditor, validateGeometryDraft } from './RoomGeometryEditor.js';
...
export class ObservatoryControlCenter {
  ...
  async refresh({ quiet = false, allowWhileBusy = false } = {}) {
    ...
    if (!quiet) {
      this.message = 'Control Center wird aktualisiert.';
      this.error = '';
    ...
    try {
      ...
      this.error = serverReached
        ? 'Control-Center-Daten sind nur teilweise verfügbar. Erneut aktualisieren oder den Serverstatus prüfen.'
        : 'Sensing-Server nicht erreichbar. Experimentaktionen bleiben gesperrt; „Aktualisieren“ versucht die Verbindung erneut.';
    } finally {
  ...
  _render() {
    ...
    this.geometryEditor = null;
    const connection = {
      ready: { className: 'is-ready', label: 'CONTROL CENTER READY' },
      degraded: { className: 'is-degraded', label: 'CONTROL CENTER DEGRADED' },
      offline: { className: 'is-offline', label: 'CONTROL CENTER OFFLINE' },
      loading: { className: 'is-loading', label: 'LOADING' },
    }[this.connectionState] || { className: 'is-loading', label: 'LOADING' };
      ...
      <section class="occ" aria-labelledby="occTitle">
        <header class="occ-header">
          <div class="occ-header-copy">
            <div class="occ-eyebrow">OBSERVATORY CONTROL CENTER / WIFI MODEL + SEPARATE REFERENCE ${infoTip('Getrennte Referenz', 'Das Cockpit steuert WiFi-CSI. mmWave liefert nur während Kalibrierung und Blindtest zeitlich zugeordnete Referenzkoordinaten und wird nicht in das WiFi-Modell eingespeist.')}</div>
            <h3 id="occTitle">Experiment cockpit</h3>
            <p>Setup, Aufnahmen, Runs und Diagnostik an einer Stelle. Der aktuelle Hardwarezustand bleibt sichtbar, ohne Ergebnisse vor der Validierung vorwegzunehmen.</p>
          </div>
        ...
        <section class="occ-panel occ-wide-panel occ-room-panel">
          ${panelHeading('ROOM EDITOR', 'Setup-Profil', 'Setup-Profil', 'Speichert Raumachsen und Node-Positionen als versioniertes Dokument. Ein altes P01–P09-Raster kann nur noch optional für Kompatibilität und Kontrolltests hinterlegt werden.')}
          <p class="occ-panel-intro">Die Koordinaten werden in Metern geführt. Achsenkonvention: <strong>x / y / z</strong>.</p>
          ...
          <form id="occProfileForm">
            ...
            <div class="occ-room-form-grid">
              ...
              <div class="occ-form-section">
                <div class="occ-subheading">Raum [Länge / Höhe / Breite] (m) ${infoTip('Raumdimensionen', 'Die Reihenfolge entspricht der Profildefinition: Länge, Höhe, Breite.')}</div>
                <div class="occ-triple">${this._tripleInputs('room_dimensions_m', this.profileDraft.room_dimensions_m)}</div>
              </div>
              <div class="occ-form-section">
                <div class="occ-subheading">TX-Position [x / y / z] (m) ${infoTip('TX-Position', 'Position der WiFi-Sendequelle im Raumkoordinatensystem. Der mmWave-Transform wird später auf dasselbe System ausgerichtet.')}</div>
                <div class="occ-triple">${this._tripleInputs('transmitter.position_m', this.profileDraft.transmitter?.position_m)}</div>
              </div>
              <div class="occ-form-section">
                <div class="occ-subheading">Receiver-Positionen [x / y / z] (m) ${infoTip('Receiver-Positionen', 'Die vier RX-Knoten werden als feste Messgeometrie im Setup-Profil gespeichert.')}</div>
              ...
              <div class="occ-room-form-column">
                <div class="occ-calibration-route">
                  <div class="occ-route-kicker">KALIBRIERUNGSQUELLE</div>
                  ...
                  <h5>Kontinuierliche Radar-Referenz</h5>
                  <p>Während einer CSI-Aufnahme liefert der Radar laufend x/z-Koordinaten. Damit entfällt das manuelle Abarbeiten von P01 bis P09.</p>
                  <button type="button" class="occ-button occ-button-primary" data-occ-action="open-mmwave-calibration">Zum mmWave-Assistenten</button>
                  <p class="occ-helper">Die Radarwerte bleiben getrennte Ground Truth. Sie dürfen nicht als Eingabe in den WiFi-Prädiktor gelangen.</p>
                </div>
                <details class="occ-fold occ-coordinate-fallback" ${this.pointGridExpanded ? 'open' : ''}>
                  <summary data-occ-action="toggle-point-grid"><span>Optionales P01–P09-Referenzraster</span><small>Legacy / Kontrolltest</small></summary>
                  <p class="occ-helper">Nur für bestehende Datensätze oder reproduzierbare Checkpoints. Für die reguläre mmWave-Kalibrierung ist dieses Raster nicht erforderlich.</p>
                  ...
                  `).join('')}</div>
                  <button type="button" class="occ-button occ-button-quiet" data-occ-action="generate-points">Optionales 3×3-Raster erzeugen</button>
                </details>
              </div>
            </div>
            ...
            <p class="occ-helper">Speichern erzeugt eine neue serverseitige Profilversion. Die Raumdaten werden nicht automatisch aus der Hardware vermessen.</p>
          </form>
          <details class="occ-fold" ${this.profilesExpanded ? 'open' : ''}>
            <summary data-occ-action="toggle-profiles"><span>Gespeicherte Profile</span><small>${this.profiles.length ? `${this.profiles.length} Versionen` : 'leer'}</small></summary>
            <div class="occ-profile-list">
              ...
              `).join('') : '<div class="occ-empty">Noch kein serverseitiges Setup-Profil.</div>'}
            </div>
          </details>
        </section>

        <section class="occ-panel occ-wide-panel occ-workflow-panel">
          ${panelHeading('EXPERIMENT RUNNER', 'WiFi-Modell-Workflow', 'Workflow', 'Ein Run hält Setup-Hash, Firmware-/Dataset-Version, CSI-Aufnahmen und die getrennte mmWave-Referenz zusammen. Jeder Übergang bleibt als eigener Schritt sichtbar.')}
          ${this._workflowMarkup()}
          ${this._runHistoryMarkup()}
        </section>

        <section class="occ-panel occ-wide-panel">
          <div class="occ-section-header">${panelHeading('NODE INVENTORY', 'TX / RX Status', 'Node-Status', 'Nur-Lesen-Übersicht aus dem Serverstatus: Erreichbarkeit, CSI-Rate, Sequenzlücken und Synchronisationszustand.')}<span class="occ-note">Nur Beobachtung · keine Firmware-Schreibaktion</span></div>
          ${this._nodesMarkup()}
        </section>

        <div class="occ-grid occ-lower-grid">
          <section class="occ-panel">${panelHeading('CAPTURE CATALOGUE', 'Aufnahmen', 'Aufnahmekatalog', 'Katalog der serverseitig gefundenen CSI-Aufnahmen. Rohdaten bleiben getrennt von Ground Truth und Modellartefakten.')} ${this._recordingsMarkup()}</section>
          <section class="occ-panel">${panelHeading('MODEL & BENCHMARK', 'Vergleichsbasis', 'Benchmark-Katalog', 'Zeigt nur den definierten Vergleichsvertrag. Aussagekräftige Modellwerte entstehen erst nach neuen gelabelten WiFi-Captures und einem abgeschlossenen Blindtest.')} ${this._benchmarkMarkup()}</section>
        </div>
      </section>
    `;
  ...
  _overviewMarkup() {
    ...
    return [
      ['Aktive Nodes', `${active}/${nodes.length || 4}`, active === 4 ? 'is-good' : 'is-warn', 'Aktive Nodes', 'Online-Zahl aus dem letzten Serverstatus. Das sagt noch nichts über Signalqualität oder Blindtest-Tauglichkeit aus.'],
      ['TX-Quelle', txAttested ? `${txAttested} RX attestiert` : 'unbekannt', txAttested ? 'is-good' : 'is-warn', 'TX-Quellenbindung', 'Zeigt, ob die RX-Seite die erwartete TX-Quelle im aktuellen Serverzustand bestätigt.'],
      ['Präsenzkalibrierung', this.status?.classification_calibration?.phase || '--', this.status?.classification_calibration?.phase === 'ready' ? 'is-good' : 'is-warn', 'Präsenzkalibrierung', 'Status der WiFi-Präsenzkalibrierung. Dieser Wert ist unabhängig von mmWave.'],
      ['Workflow', current ? phaseLabel(current.current_phase) : 'kein Run', current?.current_status === 'PASS' ? 'is-good' : '', 'Workflow-Schritt', 'Aktiver Schritt des ausgewählten Experiment-Runs; gesperrte Schritte warten auf ihre Vorbedingungen.'],
      ['mmWave-Referenz', this.status?.mmwave?.packets_received > 0 ? 'verbunden' : 'nicht verbunden', 'is-locked', 'mmWave-Status', 'Der Radar liefert Kalibrierungs- und Blindtest-Referenzkoordinaten, bleibt aber außerhalb des WiFi-Modells. Ohne echte Pakete ist dieser Pfad nicht validiert.'],
      ['Runs', String(this.runs.length), '', 'Experiment-Runs', 'Anzahl der serverseitig gespeicherten Experimentläufe.'],
    ...
    if (!this.selectedRun) {
      if (!this.selectedProfile) {
        return {
          ...
          body: 'Lege Raumachsen sowie TX- und RX-Positionen fest. Das Profil ist die unveränderliche Geometrie-Referenz für den späteren Run.',
          checklist: ['Raummaße prüfen', 'TX/RX-Positionen eintragen', 'Profil speichern'],
          action: 'focus-profile',
          actionLabel: 'Zum Setup-Profil',
          navigationOnly: true,
          helper: 'Die Koordinaten werden nicht automatisch aus der Hardware vermessen.',
        };
      }
      return {
        progress: 'VORBEREITUNG',
        ...
        body: 'Der Run bindet den Profil-Hash, Dataset-/Firmware-Version und später die getrennte mmWave-Referenz.',
        checklist: ['Profil ausgewählt', 'Versuchsname vergeben', 'Run anlegen'],
        action: 'focus-workflow',
        actionLabel: 'Zum Run-Formular',
        navigationOnly: true,
        helper: 'Danach führt dich der Guide automatisch durch die Workflow-Phasen.',
      };
    }
  ...
  _workflowMarkup() {
    if (!this.selectedRun) {
      return `
        ${this._guideMarkup()}
        <p class="occ-copy">Ein Run bindet Profil-Hash, Dataset-/Firmware-Version und einen Blindtest-Seed. Die Hardwareaktionen bleiben einzeln sichtbar und nachvollziehbar.</p>
        <form id="occWorkflowForm">
          ...
          <label class="occ-field"><span>Setup-Profil ${infoTip('Setup-Profil', 'Der Run übernimmt den Hash dieser Profilversion. Nach dem Versiegeln darf die Geometrie für diesen Run nicht still geändert werden.')}</span><select name="profile_id" ${this.profiles.length ? '' : 'disabled'}>${this.profiles.map((profile) => `<option value="${attribute(profile.id)}" ${profile.id === this.selectedProfile?.id ? 'selected' : ''}>${escapeHTML(profile.label)} · ${escapeHTML(profile.profile_sha256.slice(0, 12))}…</option>`).join('')}</select></label>
          <div class="occ-inline-actions"><button type="submit" class="occ-button occ-button-primary" ${this.profiles.length && !this.busy && this.connectionState === 'ready' ? '' : 'disabled'}>WiFi-Experiment anlegen</button></div>
        </form>
        <p class="occ-helper">${this.profiles.length ? 'Nächster Schritt: Setup software-seitig versiegeln.' : 'Zuerst ein Setup-Profil speichern.'}</p>
      `;
    }
    const workflow = this.selectedRun.workflow;
    ...
    if (workflow.current_phase === 'train_p01_p09') {
      ...
      return `
        <div class="occ-calibration-route occ-workflow-route">
          ...
          <h5>CSI mit kontinuierlichen mmWave-Koordinaten kalibrieren</h5>
          <p>Der Assistent koppelt CSI-Zeitfenster an die separat gespeicherte Radarposition. P01–P09 müssen dabei nicht manuell aufgenommen werden.</p>
          <button type="button" class="occ-button occ-button-primary" data-occ-action="open-mmwave-calibration">mmWave-Kalibrierung öffnen</button>
        </div>
        <details class="occ-fold occ-manual-fallback" ${this.manualCaptureExpanded ? 'open' : ''}>
          <summary data-occ-action="toggle-manual-capture"><span>Manuelle Punktaufnahme</span><small>Fallback für Legacy-Runs</small></summary>
  ...
  _nodesMarkup() {
    const nodes = Array.isArray(this.status?.nodes) ? this.status.nodes : [];
    if (!nodes.length) return '<div class="occ-empty">Noch keine live CSI-Nodes im Serverzustand.</div>';
    return `<div class="occ-table-wrap"><table class="occ-table"><thead><tr><th>Node</th><th>Status</th><th>RSSI ${infoTip('RSSI', 'Empfangsstärke des zuletzt sichtbaren Pakets; kein Maß für die Modellgenauigkeit.')}</th><th>CSI-Rate ${infoTip('CSI-Rate', 'Geschätzte Frames pro Sekunde aus dem bisherigen Serverfenster.')}</th><th>Verlustschätzung ${infoTip('Verlustschätzung', 'Aus Sequenzlücken abgeleitete Schätzung. Sie ersetzt keine Ende-zu-Ende-Paketverlustmessung.')}</th><th>Sequenz</th><th>Zeit / Sync ${infoTip('Zeit / Sync', 'Zeigt, ob der Zeitversatz zum Mesh-Zeitbezug aktuell und gültig ist.')}</th></tr></thead><tbody>${nodes.map((node) => `
  ...
  _recordingsMarkup() {
    if (!this.recordings.length) return '<div class="occ-empty">Keine CSI-Aufnahmen katalogisiert.</div>';
  ...
  _benchmarkMarkup() {
    ...
    return `<div class="occ-benchmark-grid"><div><span>Aktives Modell</span><strong>${escapeHTML(activeModel)}</strong></div><div><span>Gefundene RVF</span><strong>${this.models.length}</strong></div><div><span>Baseline</span><strong>${escapeHTML(catalog.baseline?.id || 'prototype_d6')}</strong></div><div><span>Vergleichsmodelle</span><strong>${comparators.map((model) => escapeHTML(model.id)).join(' · ') || 'wird geladen'}</strong></div><div><span>Train/Test-Split</span><strong>${escapeHTML(catalog.split?.id || 'sealed_wifi_train_blind_test_v1')}</strong></div><div><span>RX-Ablation</span><strong>${ablation.length || 5} Varianten</strong></div></div><p class="occ-helper">${catalog.status === 'READY_FOR_WIFI_DATA' ? 'Vertrag bereit. Ohne neue gelabelte WiFi-Captures werden hier keine Modellwerte behauptet.' : 'Benchmark-Katalog wird geladen.'} mmWave bleibt unabhängige Referenz und ist kein Modelleingang.</p>`;
  }
