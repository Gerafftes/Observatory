...
export class TabManager {
  ...
  init() {
    ...
    if (nav) {
      nav.addEventListener('keydown', (e) => {
        const buttonTabs = this.tabs.filter(t => t.tagName === 'BUTTON' && !t.disabled && !t.closest('details:not([open])'));
  ...
  switchTab(tabElement) {
    ...
    const moreMenu = this.container.querySelector('.nav-more');
    if (moreMenu) moreMenu.classList.toggle('has-active', Boolean(tabElement.closest('.nav-more')));
