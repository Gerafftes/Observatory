...
class Observatory {
  constructor() {
    ...
    this._liveConnectionState = 'closed';
  ...
  _buildRoom() {
    ...
    this._table.position.set(-4, 0.3, -3);
  ...
  // ---- Validated hardware scene ----

  _buildHardwareScene() {
    this._hardwareGroup = new THREE.Group();
    this._hardwareGroup.visible = false;
    this._scene.add(this._hardwareGroup);

    this._hardwareEnvironment = new THREE.Group();
    this._hardwareGroup.add(this._hardwareEnvironment);
    this._hardwareGeometryKey = null;
    this._hardwareFieldKey = null;
    this._hardwareFieldPoints = null;

    this._hardwarePositionMarker = new THREE.Group();
    ...
    this._hardwarePositionMarker.add(markerRing);

    const markerMast = new THREE.Mesh(
    ...
    markerMast.position.y = 0.425;
    this._hardwarePositionMarker.add(markerMast);

    const markerHead = new THREE.Mesh(
      new THREE.SphereGeometry(0.075, 16, 12),
      new THREE.MeshBasicMaterial({ color: C.greenBright })
    );
    markerHead.position.y = 0.88;
    this._hardwarePositionMarker.add(markerHead);
    this._hardwarePositionMarker.visible = false;
    this._hardwareGroup.add(this._hardwarePositionMarker);
  }
  ...
  _rebuildHardwareEnvironment(geometry) {
    const geometryKey = JSON.stringify({
      room: geometry.roomDimensions,
      tx: geometry.txPosition,
      receivers: geometry.receivers,
    });
    if (geometryKey === this._hardwareGeometryKey) return;

    this._hardwareGroup.remove(this._hardwareEnvironment);
    this._disposeObject(this._hardwareEnvironment);
    this._hardwareEnvironment = new THREE.Group();
    this._hardwareGroup.add(this._hardwareEnvironment);
    this._hardwareGeometryKey = geometryKey;
    this._hardwareFieldKey = null;

    const [length, height, width] = geometry.roomDimensions;
    ...
    roomEdges.position.set(length / 2, height / 2, width / 2);
    roomEdges.visible = this.settings.room;
    this._hardwareEnvironment.add(roomEdges);
    this._hardwareRoomWire = roomEdges;

    const gridSpan = Math.max(length, width);
    const grid = new THREE.GridHelper(gridSpan, 20, 0x1a4830, 0x0c2818);
    grid.scale.set(length / gridSpan, 1, width / gridSpan);
    grid.position.set(length / 2, 0, width / 2);
    grid.material.opacity = 0.5;
    grid.material.transparent = true;
    grid.visible = this.settings.grid;
    this._hardwareEnvironment.add(grid);
    this._hardwareGrid = grid;
    ...
    this._hardwareEnvironment.add(floor);
    this._hardwareFloorMat = floorMaterial;
    ...
    this._controls.target.set(length / 2, Math.min(height / 2, 1.3), width / 2);
  ...
  _rebuildHardwareField(field, geometry) {
    const fieldKey = `${field.columns}x${field.rows}:${geometry.roomDimensions.join(',')}`;
    if (
      fieldKey === this._hardwareFieldKey &&
      this._hardwareFieldPoints
    ) {
      return;
    }

    if (this._hardwareFieldPoints) {
      this._hardwareGroup.remove(this._hardwareFieldPoints);
      this._disposeObject(this._hardwareFieldPoints);
    }
    ...
    for (let iz = 0; iz < field.rows; iz++) {
      for (let ix = 0; ix < field.columns; ix++) {
        const index = iz * field.columns + ix;
        positions[index * 3] = ix * field.cellSizeX;
        positions[index * 3 + 1] = 0.025;
        positions[index * 3 + 2] = iz * field.cellSizeZ;
      }
    }
    ...
    this._hardwareFieldPoints = new THREE.Points(
      geometryBuffer,
      new THREE.PointsMaterial({
        size: Math.max(field.cellSizeX, field.cellSizeZ) * 0.55,
        vertexColors: true,
        transparent: true,
        opacity: this.settings.field,
        blending: THREE.AdditiveBlending,
        depthWrite: false,
        sizeAttenuation: true,
      })
    );
    this._hardwareFieldPoints.visible = false;
    this._hardwareGroup.add(this._hardwareFieldPoints);
    this._hardwareFieldKey = fieldKey;
  }

  _updateHardwareField(field) {
    const colors = this._hardwareFieldPoints.geometry.attributes.color;
    for (let index = 0; index < field.values.length; index++) {
      const value = Math.min(1, Math.max(0, field.values[index]));
      let red;
      let green;
      let blue;
      if (value < 0.3) {
      ...
      colors.array[index * 3] = red;
      colors.array[index * 3 + 1] = green;
      colors.array[index * 3 + 2] = blue;
    }
    colors.needsUpdate = true;
  }

  _updateHardwareScene(contract) {
    if (!contract?.geometry?.valid) {
      this._hardwareGroup.visible = false;
      this._hardwarePositionMarker.visible = false;
      if (this._hardwareFieldPoints) this._hardwareFieldPoints.visible = false;
      return;
    }

    this._rebuildHardwareEnvironment(contract.geometry);
    this._hardwareGroup.visible = true;
    this._hardwareRoomWire.visible = this.settings.room;
    this._hardwareGrid.visible = this.settings.grid;

    this._hardwarePositionMarker.visible = contract.showHardwareMarker;
    if (contract.showHardwareMarker) {
      this._hardwarePositionMarker.position.fromArray(
        contract.position.coordinates
      );
    }

    if (contract.showHardwareField) {
      this._rebuildHardwareField(contract.signalField, contract.geometry);
      this._updateHardwareField(contract.signalField);
      this._hardwareFieldPoints.material.opacity = this.settings.field;
      this._hardwareFieldPoints.visible = true;
    } else if (this._hardwareFieldPoints) {
      this._hardwareFieldPoints.visible = false;
    }
  }

  _setSceneMode(mode) {
    const showDemo = mode === 'simulated';
    this._grid.visible = showDemo && this.settings.grid;
    this._roomWire.visible = showDemo && this.settings.room;
    this._floor.visible = showDemo;
    this._table.visible = showDemo;
    this._routerGroup.visible = showDemo;
    this._mistPoints.visible = showDemo;
    this._trail.visible = showDemo;
    this._fieldPoints.visible = showDemo;
    for (const wave of this._wifiWaves) wave.mesh.visible = showDemo;
    ...
    if (mode !== this._sceneMode && showDemo) {
      this._controls.target.set(0, 1.2, 0);
      this._controls.update();
    }
  ...
  _resolveViewState(dt, nowMs) {
    const sourceState = resolveObservatorySourceState({
      selectedSource: this.settings.dataSource,
      connectionState: this._liveConnectionState,
      connectionOpenedAtMs: this._liveConnectionOpenedAtMs,
      frame: this._liveData,
      receivedAtMs: this._liveDataReceivedAtMs,
      nowMs,
    });

    if (sourceState.state === 'simulated') {
      const liveSimulation = this.settings.dataSource === 'ws' && this._liveData;
      return {
        sourceState,
        data: liveSimulation ? this._liveData : this._demoData.update(dt),
        contract: null,
      };
    }
    ...
    if (sourceState.state === 'live') {
      return {
        sourceState,
        data: this._liveData,
        contract: resolveObservatoryRenderContract(this._liveData),
      };
    }
  ...
  _connectWS(url) {
    this._disconnectWS();
    this._liveConnectionState = 'connecting';
    this._liveData = null;
    this._liveDataReceivedAtMs = null;
    this._liveConnectionOpenedAtMs = null;
    try {
      ...
      socket.onopen = () => {
        ...
        this._liveConnectionState = 'open';
        this._liveConnectionOpenedAtMs = performance.now();
      };
      socket.onmessage = (evt) => {
        if (this._ws !== socket) return;
        try {
          this._liveData = JSON.parse(evt.data);
          this._liveDataReceivedAtMs = performance.now();
        } catch {}
      };
      socket.onclose = () => {
        if (this._ws !== socket) return;
  ...
  _animate() {
    ...
    const viewState = this._resolveViewState(dt, performance.now());
    this._currentViewState = viewState;
    this._currentData = viewState.data;
    const data = viewState.data;
    const simulated = viewState.sourceState.state === 'simulated';
    const live = viewState.sourceState.state === 'live';
    const emptyHardwareFrame = {
    ...
    this._setSceneMode(simulated ? 'simulated' : (live ? 'live' : 'inactive'));
    this._scenarioProps.setEnabled(simulated);
    this._figurePool.update(simulated ? data : emptyHardwareFrame, elapsed);
    this._scenarioProps.update(
      simulated ? data : emptyHardwareFrame,
      simulated ? this._demoData.currentScenario : '__hardware__'
    );
    this._updateDotMatrixMist(simulated ? data : emptyHardwareFrame, elapsed);
    this._updateParticleTrail(
      simulated ? data : emptyHardwareFrame,
      dt,
      elapsed
    );
    this._updateWifiWaves(elapsed);
    if (simulated) this._updateSignalField(data);
    if (live) this._updateHardwareScene(viewState.contract);
    this._hud.updateHUD(data, this._demoData, viewState);
