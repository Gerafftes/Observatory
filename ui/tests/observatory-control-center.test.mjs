...
test('room editor presents mmWave as the primary calibration route', () => {
  ...
  assert.match(container.innerHTML, /Kontinuierliche Radar-Referenz/);
  assert.match(container.innerHTML, /Zum mmWave-Assistenten/);
  assert.match(container.innerHTML, /Optionales P01–P09-Referenzraster/);
  assert.match(container.innerHTML, /Legacy \/ Kontrolltest/);
...
test('position workflow keeps manual P01 capture behind a legacy fallback', () => {
  ...
  assert.match(markup, /CSI mit kontinuierlichen mmWave-Koordinaten kalibrieren/);
  assert.match(markup, /Manuelle Punktaufnahme/);
  assert.match(markup, /Fallback für Legacy-Runs/);
...
test('guide starts with setup navigation before a run exists', () => {
  ...
  assert.match(markup, /Raummaße prüfen/);
});
...
test('benchmark panel has no hidden score claims before WiFi data exists', () => {
  ...
  assert.match(markup, /READY_FOR_WIFI_DATA|Vertrag bereit/);
  assert.match(markup, /prototype_d6/);
  assert.match(markup, /sealed_wifi_train_blind_test_v1/);
  assert.match(markup, /keine Modellwerte behauptet/);
...
test('unclear cockpit metrics expose keyboard-focusable explanations', () => {
  ...
  assert.match(markup, /Online-Zahl aus dem letzten Serverstatus/);
  assert.match(markup, /bleibt aber außerhalb des WiFi-Modells/);
});
