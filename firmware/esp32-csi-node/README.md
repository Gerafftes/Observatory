...
[![Binary: ~1104 KiB](https://img.shields.io/badge/binary-~1104%20KiB-orange.svg)](#memory-budget)
...
### 0. Archived pre-built binaries (v0.6.7 — legacy only)

Pre-built binaries are in `firmware/esp32-csi-node/release_bins/` (version: see `release_bins/version.txt`).
They predate the v0.7.0 TX source-binding packet trailer and **must not be used for
controlled localization or D5/D6 measurements**. Build the current source as shown
below for those experiments. The following command is retained only for reproducing
legacy v0.6.7 deployments:
...
### 1. Build (Docker -- reproducible method)
...
| **Flash** | 8 MB | ~1104 KiB used by firmware |
...
> **Why Docker?** ESP-IDF does not work reliably from Git Bash/MSYS2 on
> Windows. Docker provides the same v5.4 toolchain as CI. A native ESP-IDF v5.4
> installation is also supported on macOS and Linux.
...
I (406) main: ESP32-S3 CSI Node (ADR-018) -- v0.7.0 -- Node ID: 1
...
| **Total** | **~35 KB** | **640 KB** | **~1104 KiB** |

- **PSRAM remaining:** 7.36 MB (available for future use)
- **Flash partition:** 2 MB OTA slot on the 8 MB layout (about 46% headroom at the current binary size)
...
| **Binary size gate** | `esp32-csi-node.bin` file size | Must be at most 1120 KiB |
