...
TX_FILTER_IDENTITY_SCHEME = "sha256-ruview-tx-filter-mac-v1"
SENSITIVE_CONFIG_ATTRS = ("password", "seed_token", "ota_psk")
OTA_PSK_STATE_MARKER = "_ota_psk_configured"
...
def filter_mac_bytes(value: str) -> bytes:
    """Return the exact six bytes written to the NVS filter_mac blob."""
    parts = value.split(":")
    if (
        len(parts) != 6
        or any(len(part) != 2 for part in parts)
        or any(not all(character in "0123456789abcdefABCDEF" for character in part)
               for part in parts)
    ):
        raise ValueError("filter MAC must use AA:BB:CC:DD:EE:FF format")
    return bytes(int(part, 16) for part in parts)


def filter_identity_sha256(value: str) -> str:
    """Hash only the six binary bytes, matching the sealed setup contract."""
    return hashlib.sha256(filter_mac_bytes(value)).hexdigest()


def has_sensitive_config(args) -> bool:
    """Return True when generated NVS content contains secret material."""
    return any(getattr(args, name, None) is not None for name in SENSITIVE_CONFIG_ATTRS)


def write_private_no_clobber(path: str, data, *, binary: bool) -> None:
    """Create a private generated artifact and refuse to replace an old one."""
...
def ota_preservation_error(args, prior: dict):
    """Refuse a full NVS rewrite that could silently erase an OTA secret."""
    if getattr(args, "ota_psk", None) is not None:
        return None
    if getattr(args, "clear_ota_psk", False):
        return None

    known = prior.get(OTA_PSK_STATE_MARKER)
    if known is True:
        return (
            "this port state records an OTA PSK; re-supply --ota-psk or "
            "explicitly remove it with --clear-ota-psk"
        )
    if known is False:
        return None
    if getattr(args, "confirm_no_ota_psk", False):
        return None

    return (
        "no trustworthy port state proves that this device has no OTA PSK; "
        "re-supply --ota-psk, explicitly remove it with --clear-ota-psk, "
        "or pass --confirm-no-ota-psk only after verifying the device has none"
    )


def redacted_state(state: dict) -> dict:
    """Return a diagnostic state view without reusable secret/filter values."""
    redacted = dict(state)
    for field in SENSITIVE_CONFIG_ATTRS:
        if redacted.get(field) is not None:
            redacted[field] = "(set)"
    filter_mac = redacted.get("filter_mac")
    if filter_mac is not None:
        redacted["filter_mac"] = "(set)"
        redacted["filter_identity_scheme"] = TX_FILTER_IDENTITY_SCHEME
        try:
            if not isinstance(filter_mac, str):
                raise ValueError("stored filter MAC is not text")
            redacted["filter_identity_sha256"] = filter_identity_sha256(filter_mac)
        except ValueError:
            redacted["filter_identity_sha256"] = "(invalid stored value)"
    return redacted
...
def save_state(port: str, state_dir: str, state: dict) -> str:
    ...
    path = _state_path_for(port, state_dir)
    # The OTA bearer token must never be recoverable from the local merge
    # state. Only OTA_PSK_STATE_MARKER is persisted so later full NVS rewrites
    # can fail closed instead of silently deleting a configured token.
    persisted_state = dict(state)
    persisted_state.pop("ota_psk", None)
    ...
    try:
        ...
        with os.fdopen(fd, "w", encoding="utf-8") as state_file:
            fd = -1
            json.dump(persisted_state, state_file, indent=2, sort_keys=True)
            state_file.write("\n")
            state_file.flush()
...
def build_nvs_csv(args):
    ...
    if args.filter_mac is not None:
        mac_bytes = filter_mac_bytes(args.filter_mac)
...
def main():
    ...
    ota_group.add_argument(
        "--ota-psk",
        help="64-character hexadecimal OTA bearer token (written to the security NVS namespace)",
    )
    ota_group.add_argument(
        "--clear-ota-psk",
        action="store_true",
        help="explicitly remove any OTA PSK during this full NVS rewrite",
    )
    ota_group.add_argument(
        "--confirm-no-ota-psk",
        action="store_true",
        help="confirm an older port state belongs to a device with no OTA PSK",
    )
    parser.add_argument("--dry-run", action="store_true", help="Generate NVS binary but don't flash")
    parser.add_argument(
        "--allow-secret-fallback",
        action="store_true",
        help="allow a private 0600 CSV fallback when generation fails with secrets present",
    )
    ...
    if args.state:
        print(json.dumps(redacted_state(merged), indent=2, sort_keys=True))
        return

    ota_error = ota_preservation_error(args, prior)
    if ota_error is not None:
        parser.error(ota_error)
    ...
    if args.ota_psk is not None:
        if len(args.ota_psk) != 64:
            parser.error("--ota-psk must contain exactly 64 hexadecimal characters")
        try:
            bytes.fromhex(args.ota_psk)
        except ValueError:
            parser.error("--ota-psk must contain only hexadecimal characters")

    if args.ota_psk is not None:
        merged[OTA_PSK_STATE_MARKER] = True
    elif args.clear_ota_psk or args.confirm_no_ota_psk:
        merged[OTA_PSK_STATE_MARKER] = False
    ...
    if args.filter_mac is not None:
        try:
            filter_mac_bytes(args.filter_mac)
        except ValueError:
            parser.error("--filter-mac must be in AA:BB:CC:DD:EE:FF format")
    ...
    if args.filter_mac is not None:
        print("  Filter MAC:    (set)")
        print(
            f"  Filter ID:     {TX_FILTER_IDENTITY_SCHEME}:"
            f"{filter_identity_sha256(args.filter_mac)}"
        )
    if args.seed_url is not None:
    ...
    try:
        ...
        if has_sensitive_config(args) and not args.allow_secret_fallback:
            print(
                "No CSV fallback was written because the configuration contains "
                "secrets. Re-run with --allow-secret-fallback only if a durable "
                "private CSV is acceptable.",
                file=sys.stderr,
            )
            sys.exit(1)
        print("\nFallback: save a private CSV and flash manually with ESP-IDF tools.", file=sys.stderr)
        fallback_path = "nvs_config.csv"
        try:
            write_private_no_clobber(fallback_path, csv_content, binary=False)
        except FileExistsError:
            print(
                f"Refusing to overwrite existing fallback {fallback_path}",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if args.dry_run:
        ...
        try:
            write_private_no_clobber(out, nvs_bin, binary=True)
        except FileExistsError:
            print(f"Refusing to overwrite existing dry-run artifact {out}", file=sys.stderr)
            sys.exit(1)
