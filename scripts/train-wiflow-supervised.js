...
if (!args.data) {
  ...
  console.error('  --task <pose|torso>    Legacy 17-keypoint or torso-v1 mode (default: pose)');
...
const CONFIG = {
  ...
  // Architecture
  timeSteps:        20,
  numKeypoints:     args.task === 'torso' ? 4 : 17,
...
if (!['pose', 'torso'].includes(CONFIG.task)) {
  throw new Error(`Unsupported task '${CONFIG.task}'; expected pose or torso`);
}
...
function loadPairedData(filePath) {
  ...
  for (const line of lines) {
    try {
      ...
      if (CONFIG.task === 'torso') {
        if (obj.schema_version !== TORSO_SCHEMA || !obj.torso) continue;
        if (JSON.stringify(obj.rx_order) !== JSON.stringify(RX_ORDER)) continue;
        if (JSON.stringify(obj.csi_shape) !== JSON.stringify([CONFIG.timeSteps, 4, 64])) continue;
        if (!Array.isArray(obj.csi_amplitude) || obj.csi_amplitude.length !== CONFIG.timeSteps * 4 * 64) continue;

        const amplitude = timeMajorToFeatureMajor(obj.csi_amplitude, CONFIG.timeSteps, 4 * 64);
        const hasPhase = Array.isArray(obj.csi_phase);
        if (hasPhase && obj.csi_phase.length !== amplitude.length) continue;
        const phase = hasPhase
          ? timeMajorToFeatureMajor(obj.csi_phase, CONFIG.timeSteps, 4 * 64)
          : new Float32Array(0);
        const csiFlat = new Float32Array(amplitude.length + phase.length);
        csiFlat.set(amplitude);
        csiFlat.set(phase, amplitude.length);

        const kpFlat = new Float32Array(CONFIG.numKeypoints * 2);
        const confArr = new Float32Array(CONFIG.numKeypoints);
        let valid = true;
        for (let index = 0; index < TORSO_KEYPOINTS.length; index++) {
          const point = obj.torso[TORSO_KEYPOINTS[index]];
          if (!point || !Number.isFinite(point.x) || !Number.isFinite(point.y) || !Number.isFinite(point.confidence)) {
            valid = false;
            break;
          }
          kpFlat[index * 2] = point.x;
          kpFlat[index * 2 + 1] = point.y;
          confArr[index] = point.confidence;
        }
        if (!valid) continue;
        samples.push({
          csi: csiFlat,
          csiDim: csiFlat.length / CONFIG.timeSteps,
          keypoints: kpFlat,
          conf: confArr,
          timestamp: obj.ts_start || 0,
          sessionId: obj.session_id,
          phaseStatus: hasPhase ? 'sanitized' : 'unavailable',
        });
        continue;
      }
...
function normalizeTorsoSamples(trainSamples, allSamples, timeSteps) {
  const channels = trainSamples[0].phaseStatus === 'sanitized'
    ? [['amplitude', 0, 4 * 64], ['phase', 4 * 64, 8 * 64]]
    : [['amplitude', 0, 4 * 64]];
  const normalization = {};
  for (const [name, startFeature, endFeature] of channels) {
    let sum = 0;
    let sumSquares = 0;
    let count = 0;
    for (const sample of trainSamples) {
      for (let feature = startFeature; feature < endFeature; feature++) {
        for (let time = 0; time < timeSteps; time++) {
          const value = sample.csi[feature * timeSteps + time];
          sum += value;
    ...
    if (!Number.isFinite(std) || std <= 1e-8) {
      throw new Error(`${name} normalization has zero or invalid standard deviation`);
    }
    normalization[name] = { mean, std };
    for (const sample of allSamples) {
      for (let feature = startFeature; feature < endFeature; feature++) {
        for (let time = 0; time < timeSteps; time++) {
          const index = feature * timeSteps + time;
          sample.csi[index] = (sample.csi[index] - mean) / std;
        }
      }
    }
  }
...
async function main() {
  ...
  if (allSamples.some(sample => sample.csiDim !== inputDim)) {
    throw new Error('all samples must have the same phase status and input dimension');
  }
  let trainSet;
  let evalSet;
  let normalization = null;
  if (CONFIG.task === 'torso') {
    ({ train: trainSet, eval: evalSet } = splitBySession(allSamples, CONFIG.evalSplit));
    normalization = normalizeTorsoSamples(trainSet, allSamples, T);
  }
  ...
  if (CONFIG.task !== 'torso' && inputDim >= 64) {
  ...
  // -----------------------------------------------------------------------
  console.log(`  [O7] ${CONFIG.task === 'torso' ? 'Skipped for fixed torso-v1 input contract' : 'Computing subcarrier attention weights (ruvector-attention)...'}`);
  const subcarrierAttention = CONFIG.task === 'torso'
    ? new Float32Array(inputDim).fill(1)
    : computeSubcarrierAttention(allSamples, inputDim, T);
  ...
  // -----------------------------------------------------------------------
  if (CONFIG.task !== 'torso' && inputDim >= 16) {
  ...
  if (CONFIG.task !== 'torso') {
    ...
    evalSet = shuffled.slice(splitIdx);
  }
  ...
  const weightsSha256 = crypto.createHash('sha256').update(Buffer.from(weights.buffer)).digest('hex');
  const modelExport = {
    format: CONFIG.task === 'torso' ? TORSO_SCHEMA : 'wiflow-supervised-v1',
    adr: 'ADR-079',
    architecture: {
      inputDim,
      timeSteps: T,
    ...
    task: CONFIG.task,
    validationStatus: CONFIG.task === 'torso' ? 'UNVALIDATED' : undefined,
  ...
  if (CONFIG.task === 'torso') {
    const manifest = {
      ...
      input: {
        ...
        subcarriers: 64,
        rx_order: [...RX_ORDER],
        channels: allSamples[0].phaseStatus === 'sanitized' ? ['amplitude', 'phase'] : ['amplitude'],
      },
      phase_status: allSamples[0].phaseStatus,
      normalization,
      output: { keypoints: [...TORSO_KEYPOINTS], fields: ['x', 'y', 'confidence'] },
      sha256: weightsSha256,
      fall_thresholds: null,
      alarms_enabled: false,
    };
    const manifestPath = path.join(CONFIG.outputDir, 'torso-v1.manifest.json');
    fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2));
    console.log(`  Torso manifest: ${manifestPath} (UNVALIDATED; live fall gating disabled)`);
  }
